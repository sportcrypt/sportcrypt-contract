...
class SportCrypt {
    ...
    setGasPrice(gasPrice) {
        ...
        } else if (typeof(gasPrice) === 'object' && gasPrice.isBigNumber) {
    ...
    sendTX(method, args, opts, waitForMine, cb) {
        ...
        if (this.privateKey) {
            this.web3.eth.getTransactionCount(this.userAddr, 'pending', (err, transactionCount) => {
                if (err) return cb(err);

                if (this._nextNonce !== undefined && this._nextNonce > transactionCount) transactionCount = this._nextNonce;
                callObject.nonce = transactionCount;
                this._nextNonce = transactionCount + 1;
    ...
    checkOrderBatch(orders, cb) {
        ...
        async.map(batches, this.sc.checkOrderBatch.call, (err, res) => {
            ...
            for (let b of res) {
                if (b[1].filter(amount => !amount.equals(0)).length > 0) {
                    Array.prototype.push.apply(status, b[0]);
                    Array.prototype.push.apply(amount, b[1]);
                } else {
                    // All amounts were 0, indicate that this API call may have failed (FIXME: requires update to smart contract protocol)
                    Array.prototype.push.apply(status, b[0].map(i => undefined));
                    Array.prototype.push.apply(amount, b[1].map(i => undefined));
                }
            }
    ...
    checkMatchBatch(addr, matchIds, cb) {
        ...
        async.map(batches, (b, cb) => this.sc.checkMatchBatch.call(addr, b, cb), (err, res) => {
            ...
            for (let b of res) {
                if (b[0].filter(pos => !pos.equals(0)).length > 0 || b[1].filter(finalized => !!finalized).length > 0) {
                    ...
                    // All myPositions were 0, indicate that this API call may have failed (FIXME: requires update to smart contract protocol)
                    Array.prototype.push.apply(myPosition, b[0].map(i => undefined));
                    Array.prototype.push.apply(finalized, b[1].map(i => undefined));
                    Array.prototype.push.apply(finalPrice, b[2].map(i => undefined));
                }
            }
