...
const {normalizeComponent, encodeOrderArrayNoSig, buildOrder, signWithPrivateKey} = require('./scUtil.js');
...
class SportCrypt {
    ...
    createAndAttach(userAddr, cb) {
        this.userAddr = this.normalizeAddr(userAddr);

        this.SportCrypt.new({data: "0x" + this.bin, from: this.userAddr, gas: 4000000}, (err, sc) => {
    ...
    // FIXME: use privateKey from setPrivateKey
    signFinalizationMessage(privateKey, matchId, finalPrice) {
        let details = {
            contractAddr: normalizeComponent(this.contractAddr, 160),
        ...
        let sig = signWithPrivateKey(ethUtil.sha3(new Buffer(raw, 'hex')), privateKey);
    ...
    sendTX(method, args, opts, waitForMine, cb) {
        let callObject = {
            ...
            from: this.userAddr,
            data: this.sc[method].getData.apply(this, args),
            gas: 200000,
        };
