...
function validateOrderSig(order) {
    let msgHash = ethUtil.sha3(Buffer.concat([
        new Buffer("\x19Ethereum Signed Message:\n32"),
        new Buffer(order.hash, 'hex'),
    ]));

    let recovered = ethUtil.ecrecover(msgHash, parseInt(order.sig.v, 16), new Buffer(order.sig.r, 'hex'), new Buffer(order.sig.s, 'hex'));

    let recoveredAddress = ethUtil.pubToAddress(new Buffer(recovered, 'hex')).toString('hex');

    return recoveredAddress.toLowerCase() === order.details.fromAddress.toLowerCase();
}
...
function buildFinalizationMessage(contractAddr, privateKey, matchId, finalPrice) {
    let details = {
        contractAddr: normalizeComponent(contractAddr, 160),
